...
-  `git clone` is used to target an existing repository and create a clone, or copy of the target repository.
- From there, use the Source Control pane in Positron to stage, commit and push changes to GitHub.
...
- `git add <files>` (adding all the modified files into the staging area)
- `git commit -m "Updating new newbranch_name"`
- `git push origin newbranch_name`

## Merging branches

- `git switch main`: First move to the branch we want to move content into
...
## Etiquette for working on someone else branch {.smaller}
...
- Add changes into the staging area, commit and push changes to the corresponding branch into the remote repository: 
  - `git add files`
...
To go back to main branch (assuming that you were in there): `git switch main`
...
1. For merging, go to `main` branch: `git switch main`
...
When you open the conflict file in a **text editor**, you will see the conflicted part marked like this:
