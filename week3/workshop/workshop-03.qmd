...
7. Use the command `touch` or `ni` to create two files inside Tutorial3 `example.qmd` `example1.qmd`
...
```bash
cd Tutorial3
touch example.qmd example1.qmd
```
### Windows (PowerShell)
```powershell
cd Tutorial3
ni example.qmd, example1.qmd
```
:::
:::
...
::: {.callout-important}
Please replace "Firstname lastname" with your own details
:::
...
Setting up your SSH key is a **one-time operation**. There are two ways to do this: using RStudio or using the terminal. Either way will work. Pick your options using the tabs.

::: {.panel-tabset}

## Rstudio

Go to your Rstudio session and follow the next steps:
...
and follow the prompts. When it asks you to "Enter a file in which to save the key", just press `Enter` to accept the default location. When it asks you to "Enter passphrase (empty for no passphrase)", you can choose whether you would like to add a passphrase. If you do not want a passphrase, press Enter twice.

To copy your public key, you can use the following command in your terminal:
