...
schedule |>
  mutate(
    Has_Workshop = purrr::map_lgl(
      Week,
      \(week) !is.na(week) && dir.exists(file.path(paste0("week", week), "workshop"))
    ),
    Has_Tutorial = purrr::map_lgl(
      Week,
      \(week) !is.na(week) && dir.exists(file.path(paste0("week", week), "tutorial"))
    )
  ) |>
  transmute(
    Week = Week,
    ...
    Show_Workshop = Show & Has_Workshop,
    Show_Tutorial = Show & Has_Tutorial,
    Topic = if_else(!Show, Topic, glue::glue('[{{Topic}}](./week{{Week}}/index.html){target="_blank"}', .open="{{", .close="}}")),
    Workshop = if_else(!Show_Workshop, "", glue::glue('[Workshop {{{Week}}}](./week{{{Week}}}/workshop/index.html){target="_blank"}', .open="{{{", .close="}}}")),
    Tutorial = if_else(!Show_Tutorial, "", glue::glue('[Tutorial {{{Week}}}](./week{{{Week}}}/tutorial/index.html){target="_blank"}', .open="{{{", .close="}}}")),
  ) |>
  select(-Show, -Show_Workshop, -Show_Tutorial) |>
